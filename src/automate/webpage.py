...
class Interact:
    ...
    def click_provided_element(self, element):
        """
        
        """
        self.wait.until(EC.element_to_be_clickable((By.XPATH, element.xpath))).click()
    
    def press_enter(self, tagname, attribute, value, subtag=""):
        """
        Clicks a button that typically throws a ElementNotInteractable exception
        ...
        subtag : str, default ""
            subtag to look for in xpath - MUST include the starting "/"
        """
        self.wait.until(EC.visibility_of_element_located((By.XPATH, f"//{tagname}[@{attribute}='{value}']{subtag}"))).send_keys(Keys.RETURN)

    def element_exists(self, tagname, attribute, value, subtag=""):
        """
        Clicks a button that typically throws a ElementNotInteractable exception
        ...
        try:
            ...
            _ = quick_wait.until(EC.visibility_of_element_located((By.XPATH, f"//{tagname}[@{attribute}='{value}']{subtag}")))
            return True
        except:
            return False

    def get_elements(self, tagname, attribute, value, subtag=""):
        """
        
        """
        elements = self.wait.until(EC.visibility_of_all_elements_located((By.XPATH, f"//{tagname}[@{attribute}='{value}']{subtag}")))
        return elements
